...
if [[ "${world_size}" -ne 1 ]]; then
  echo "[PREF-ONLY][warn] expected 1 GPU for this script, got world_size=${world_size}"
fi
if (( rollout_batch_size % world_size != 0 )); then
  echo "[PREF-ONLY][error] rollout_batch_size=${rollout_batch_size} must be divisible by world_size=${world_size} for even split."
  exit 1
fi
if (( logprob_micro_batch_size > 0 )) && (( logprob_micro_batch_size % world_size != 0 )); then
  echo "[PREF-ONLY][error] logprob_micro_batch_size=${logprob_micro_batch_size} must be divisible by world_size=${world_size} for even backprop split."
  exit 1
fi
