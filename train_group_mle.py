...
def _objective_potential_weight(obj: GroupObjective, args: argparse.Namespace) -> float:
    coef = 0.0
    if obj.correct:
        coef += float(args.lambda_mle)
    if obj.is_mixed:
        coef += float(args.lambda_group)
    return float(obj.prompt_weight) * coef
...
def run_optimizer_step(
    *,
    ...
    for obj_idx, obj in enumerate(objectives):
        completions = list(obj.correct) + list(obj.wrong)
        if not completions:
            continue
        prompts = [obj.train_prompt for _ in completions]
        scores, valid_counts = _compute_sequence_logps_and_counts(
            model=model,
            tokenizer=tokenizer,
        ...
        correct_count = len(obj.correct)
        pos_mask = torch.zeros(scores.shape[0], dtype=torch.bool, device=device)
        pos_mask[:correct_count] = True
        valid_mask = torch.isfinite(scores) & valid_counts.gt(0)
        ...
        if obj.is_mixed and args.lambda_group > 0:
            group_mask = valid_mask.clone()
            if args.min_correct_avg_logprob is not None:
                group_mask = group_mask & (~pos_mask | (scores >= float(args.min_correct_avg_logprob)))
            if args.min_wrong_avg_logprob is not None:
                group_mask = group_mask & (pos_mask | (scores >= float(args.min_wrong_avg_logprob)))
            group_pos = group_mask & pos_mask
            group_wrong = group_mask & ~pos_mask
            if int(group_pos.sum().item()) > 0 and int(group_wrong.sum().item()) > 0:
                group_scores = scores[group_mask]
                group_pos_mask = pos_mask[group_mask]
                train_scores = _normalize_group_scores(group_scores, args)
                logits = train_scores / float(args.group_tau)
                log_pos_mass = torch.logsumexp(logits[group_pos_mask], dim=0)
                ...
                loss_term_weight += group_weight

                correct_scores = group_scores[group_pos_mask]
                wrong_scores = group_scores[~group_pos_mask]
                ...
                gap_weighted_sum += float(gap.detach().item()) * group_weight
                mass_weighted_sum += float(correct_mass.item()) * group_weight
                group_weight_sum += group_weight
                groups_used += 1
            else:
                print(
                    f"[group_mle] group skipped after filters obj={obj_idx} "
                    f"kept_correct={int(group_pos.sum().item())} kept_wrong={int(group_wrong.sum().item())}",
                    flush=True,
                )
        ...
        total_loss_weighted_sum += obj_group_loss_value * (
            float(args.lambda_group) * float(obj.prompt_weight) if obj.is_mixed else 0.0
        )
...
def run_training(args: argparse.Namespace) -> None:
    ...
    if args.hf_data_parallel and device.type == "cuda" and torch.cuda.device_count() > 1:
        ...
        print(f"[group_mle] enabled DataParallel over {torch.cuda.device_count()} GPUs", flush=True)
    ...
    print(
        f"[group_mle] dataset_layout={layout} rollout_backend={args.online_rollout_backend} "
    ...
    with rollout_log_path.open("w", encoding="utf-8", buffering=1) as rollout_log:
        for sample in source_iter:
            ...
            print(
                f"[group_mle] rollout_step={rollout_steps}/{total_steps_str} scanned={scanned} "
            ...
            for chunk_start in range(0, len(rollout_objectives), args.online_pairs_per_step):
                ...
                if not stats.update_applied:
                    print(
                        f"[group_mle] rollout_step={rollout_steps}/{total_steps_str} "
                ...
                print(
                    f"[group_mle] rollout_step={rollout_steps}/{total_steps_str} optimizer_step={updates} "
                    f"mle_loss={stats.mle_loss:.6f} group_loss={stats.group_loss:.6f} "
                    f"mean_gap={stats.mean_gap:.6f} group_correct_mass={stats.group_correct_mass:.6f} "
                ...
                if args.online_save_every_updates > 0 and updates % args.online_save_every_updates == 0:
                    ...
                    print(f"[group_mle] saved checkpoint to {ckpt_dir}", flush=True)

            print(
                f"[group_mle] rollout_step={rollout_steps}/{total_steps_str} updates_in_rollout={updates_in_rollout}",
                flush=True,
            )
            buffer = []
    ...
    print(
        f"[group_mle] finished rollout_steps={rollout_steps} optimizer_steps={updates} scanned={scanned} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Online MLE + mixed group-softmax training.")
...
def main() -> None:
    ...
    if launcher_world_size > 1:
        raise SystemExit(
            "error: train_group_mle.py is single-process. Use CUDA_VISIBLE_DEVICES plus "
