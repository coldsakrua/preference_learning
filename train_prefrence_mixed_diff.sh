#!/bin/bash
#SBATCH -o logs/pref_qwen3_1b_mixed_diff_1gpu.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
run_root=${RUN_ROOT:-outputs/pref_qwen3_1b_mixed_diff_1gpu/${run_name}}
train_out="${run_root}/train"

mkdir -p "${run_root}" "${train_out}"

echo "[PREF-MIXED-DIFF] run_root=${run_root}"
