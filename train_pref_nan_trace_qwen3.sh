...
run_root=${RUN_ROOT:-outputs/pref_nan_trace_4b_2gpu/${run_name}}
train_out="${run_root}/train"

mkdir -p "${run_root}" "${train_out}"

echo "[PREF-NAN-TRACE] run_root=${run_root}"
echo "[PREF-NAN-TRACE] launcher=python(single process) hf_data_parallel=true"
echo "[PREF-NAN-TRACE] model=${model_path} dataset=${dataset_path}"

python train_preference_pref_only_nan_trace.py \
  --seed "${seed}" \
  ...
  --lambda_pref 1.0 \
  --nan_trace_verbose true

echo "[PREF-NAN-TRACE] done"
