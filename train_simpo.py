...
def _online_run_preference_optimizer_step(
    ...
    def _check_chunk_and_backward(
        ...
        failed = _check_loss_chunk_value(
    ...
    def _check_pseudo_chunk_and_backward(
        loss_chunk: torch.Tensor,
        skip_prefix: str,
        start: int,
        end: int,
        ...
        if args.online_skip_nonfinite_loss and not torch.isfinite(loss_chunk.detach()):
            optimizer.zero_grad(set_to_none=True)
            return _build_zero_stats(f"nonfinite_{skip_prefix}_pseudo_loss_chunk(start={start},end={end})")
        loss_chunk.backward()
        return None
    ...
    def _run_pref_like_branch(
        ...
        for start in range(0, branch_batch, micro_batch):
            ...
            if chunk_pairs_used <= 0:
                continue
            if args.simpo_low_vram_backward:
                was_training = bool(getattr(model, "training", False))
                if was_training:
                    model.eval()
                try:
                    with torch.no_grad():
                        chosen_probe_logps = _compute_sequence_logps_batch(
                            model,
                            tokenizer,
                            tp,
                            ch,
                        ...
                        rejected_probe_logps = _compute_sequence_logps_batch(
                            model,
                            tokenizer,
                            tp,
                            rj,
                ...
                preference_gap = chosen_probe_logps - rejected_probe_logps
                if args.online_gap_clip_abs > 0:
                    preference_gap = preference_gap.clamp(-args.online_gap_clip_abs, args.online_gap_clip_abs)
                pref_like_loss_vec = F.softplus(float(args.simpo_margin) - float(args.beta) * preference_gap)
                ...
                simpo_coef = (
                    float(args.beta)
                    * torch.sigmoid(float(args.simpo_margin) - float(args.beta) * preference_gap)
                ).detach()
                chosen_logps = _compute_sequence_logps_batch(
                    model,
                    tokenizer,
                ...
                chosen_pseudo_loss = (-(simpo_coef * chosen_logps) * w).sum() / total_weight
                failed = _check_pseudo_chunk_and_backward(
                    loss_chunk=chosen_pseudo_loss,
                    skip_prefix=f"{branch}_chosen",
                    start=start,
                    end=end,
                )
                if failed is not None:
                    return failed
                del chosen_logps, chosen_pseudo_loss
                ...
                rejected_pseudo_loss = ((simpo_coef * rejected_logps) * w).sum() / total_weight
                failed = _check_pseudo_chunk_and_backward(
                    loss_chunk=rejected_pseudo_loss,
                    skip_prefix=f"{branch}_rejected",
                    start=start,
                    end=end,
                )
                if failed is not None:
                    return failed
                del rejected_logps, rejected_pseudo_loss, simpo_coef
            else:
                chosen_logps = _compute_sequence_logps_batch(
                    model,
                    tokenizer,
...
def run_online_preference_training(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"simpo_margin={args.simpo_margin}, simpo_low_vram_backward={args.simpo_low_vram_backward}, "
    ...
    _write_metric(
        "run_start",
        {
            ...
            "beta": args.beta,
            "simpo_margin": args.simpo_margin,
            "simpo_low_vram_backward": args.simpo_low_vram_backward,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Margin gamma in SimPO loss: softplus(gamma - beta * (chosen_avg_logp - rejected_avg_logp)).",
    )
    parser.add_argument(
        "--simpo_low_vram_backward",
        type=str2bool,
        default=True,
        help=(
            "Compute SimPO coefficients with no-grad forwards, then backprop chosen/rejected separately. "
            "This uses extra forward time but avoids keeping both trajectory graphs in memory."
        ),
    )
    ...
    if launcher_world_size > 1:
        raise SystemExit(
            "error: train_simpo.py is a single-process online trainer. "
    ...
    validations = [
        ...
        (args.simpo_margin < 0, "error: --simpo_margin must be >= 0"),
