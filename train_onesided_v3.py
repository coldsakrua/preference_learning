#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Online One-Sided Answer-Cluster MLE - v3.

Compared to ``train_onesided_v2.py``, this file changes the mixed/all-wrong
signal from detached one-sided reweighting to answer-cluster contrast:
  ...
  2. Answer-cluster contrast on wrong final answers (``--lambda_answer``)
     - wrong rollouts are clustered by their normalized final answer and enter
       a softmax denominator against the ground-truth answer. This uses
       mixed/all-wrong prompts without treating whole wrong trajectories as
       uniformly bad reasoning.
...
Default ``--lambda_mle`` is lowered so the answer-contrast branch can carry a
meaningful share of the update. ``--prompt_weight_gamma`` is left at the
legacy default (1.0) so that the only difficulty signal used
is the on-policy rollout correct rate; no external difficulty prior is assumed
about the dataset.
...
def _answer_cluster_weight(count: int, args: argparse.Namespace) -> float:
    mode = str(args.answer_cluster_weight)
    if mode == "uniform":
        return 1.0
    if mode == "count":
        return float(max(1, count))
    if mode == "sqrt_count":
        return math.sqrt(float(max(1, count)))
    raise ValueError(f"Unsupported --answer_cluster_weight: {mode}")


def _answer_text_for_scoring(answer: str, args: argparse.Namespace) -> str:
    text = normalize_answer_robust(answer) if args.answer_use_normalized else str(answer).strip()
    return text.strip()


def _answer_completion(answer: str, args: argparse.Namespace) -> str:
    return f"{args.answer_completion_prefix}{answer}{args.answer_completion_suffix}"


def _build_wrong_answer_clusters(
    wrong_responses: Sequence[str],
    ground_truth: str,
    args: argparse.Namespace,
) -> List[AnswerCluster]:
    gt_key = normalize_answer_robust(ground_truth)
    counts: Counter = Counter()
    for response in wrong_responses:
        has_final, parsed = extract_rollout_scored_answer(response or "")
        if not has_final or not parsed:
            continue
        if answer_text_matches_robust(parsed, ground_truth):
            continue
        key = _answer_text_for_scoring(parsed, args)
        if not key:
            continue
        if gt_key and normalize_answer_robust(key) == gt_key:
            continue
        counts[key] += 1

    min_count = int(args.answer_min_cluster_count)
    clusters = [
        AnswerCluster(answer=answer, count=count)
        for answer, count in counts.items()
        if count >= min_count
    ]
    clusters.sort(key=lambda c: (-c.count, c.answer))
    if args.answer_max_clusters > 0:
        clusters = clusters[: int(args.answer_max_clusters)]
    return clusters


def _compute_answer_contrast_loss(
    *,
    model: object,
    tokenizer: object,
    device: torch.device,
...
) -> Tuple[Optional[torch.Tensor], float, float, int, int]:
    if not clusters:
        return None, 0.0, 0.0, 0, 0
    positive_answer = _answer_text_for_scoring(ground_truth, args)
    if not positive_answer:
        return None, 0.0, 0.0, 0, 0

    answers = [positive_answer] + [cluster.answer for cluster in clusters]
    prompts = [train_prompt for _ in answers]
    completions = [_answer_completion(answer, args) for answer in answers]
    scores, valid_counts = _compute_seq_logps_token_weighted(
        model=model,
        tokenizer=tokenizer,
    ...
    valid = torch.isfinite(scores) & valid_counts.gt(0)
    ...
    for idx, cluster in enumerate(clusters, start=1):
        ...
        neg_weights.append(_answer_cluster_weight(cluster.count, args))
        neg_count_total += int(cluster.count)
        cluster_count += 1
    if not neg_scores:
        return None, 0.0, 0.0, 0, 0

    pos_logit = scores[0] / float(args.answer_tau)
    neg_logit_tensor = torch.stack(neg_scores) / float(args.answer_tau)
    weight_tensor = torch.tensor(
        neg_weights, device=device, dtype=neg_logit_tensor.dtype
    ).clamp_min(1e-6)
    all_logits = torch.cat([pos_logit.view(1), neg_logit_tensor + weight_tensor.log()])
    denom = torch.logsumexp(all_logits, dim=0)
    loss = denom - pos_logit
    correct_prob = float(torch.exp(pos_logit.detach() - denom.detach()).item())
    return loss, float(loss.detach().item()), correct_prob, cluster_count, neg_count_total
...
@dataclass
class OptimizerStepStatsV3:
    total_loss: float
    mle_loss: float
    answer_loss: float
    answer_correct_prob: float
    mle_samples_used: int
    answer_clusters_used: int
    answer_wrong_answers_used: int
    answer_objects_used: int
    grad_norm: float
    update_applied: bool
    skip_reason: str
...
def _zero_stats_v3(reason: str = "", grad_norm: float = 0.0) -> OptimizerStepStatsV3:
    return OptimizerStepStatsV3(
        total_loss=0.0,
        mle_loss=0.0,
        answer_loss=0.0,
        answer_correct_prob=0.0,
        mle_samples_used=0,
        answer_clusters_used=0,
        answer_wrong_answers_used=0,
        answer_objects_used=0,
        grad_norm=grad_norm,
...
def run_optimizer_step_v3(
    *,
...
) -> OptimizerStepStatsV3:
    prepared: List[Tuple[GroupObjective, List[AnswerCluster], float]] = []
    total_weight = 0.0
    for obj in objectives:
        answer_clusters = _build_wrong_answer_clusters(obj.wrong, obj.ground_truth, args)
        obj_weight = 0.0
        if obj.correct and args.lambda_mle > 0:
            obj_weight += float(args.lambda_mle) * float(obj.prompt_weight)
        if answer_clusters and args.lambda_answer > 0:
            obj_weight += float(args.lambda_answer) * float(obj.prompt_weight)
        prepared.append((obj, answer_clusters, obj_weight))
        total_weight += obj_weight
    if total_weight <= 0:
        return _zero_stats_v3("zero_total_weight")

    optimizer.zero_grad(set_to_none=True)

    total_loss_w = 0.0
    mle_loss_w_sum = 0.0
    answer_loss_w_sum = 0.0
    answer_prob_w_sum = 0.0
    mle_w_total = 0.0
    answer_w_total = 0.0
    mle_samples_used = 0
    answer_clusters_used = 0
    ...
    for obj_idx, (obj, answer_clusters, _obj_weight) in enumerate(prepared):
        pos_valid: Optional[torch.Tensor] = None
        ...
        obj_mle_val = 0.0
        obj_answer_val = 0.0
        obj_answer_used = False

        n_pos = int(pos_valid.sum().item()) if pos_valid is not None else 0
        if n_pos > 0 and args.lambda_mle > 0 and pos_scores is not None:
        ...
        if answer_clusters and args.lambda_answer > 0:
            (
                ...
                cluster_count,
                wrong_answer_count,
            ) = _compute_answer_contrast_loss(
                model=model,
                tokenizer=tokenizer,
                device=device,
                args=args,
                train_prompt=obj.train_prompt,
                ground_truth=obj.ground_truth,
                clusters=answer_clusters,
            )
            if answer_loss is not None and cluster_count > 0:
                answer_wt = float(args.lambda_answer) * float(obj.prompt_weight)
                loss_terms.append(answer_loss * answer_wt)
                obj_answer_used = True
                obj_answer_val = answer_val
                answer_loss_w_sum += answer_val * answer_wt
                answer_prob_w_sum += answer_prob * answer_wt
                answer_w_total += answer_wt
                answer_clusters_used += cluster_count
                answer_wrong_answers_used += wrong_answer_count
                answer_objects_used += 1
        ...
        total_loss_w += obj_answer_val * (
            float(args.lambda_answer) * float(obj.prompt_weight) if obj_answer_used else 0.0
        )

    if mle_w_total + answer_w_total <= 0:
        optimizer.zero_grad(set_to_none=True)
        return _zero_stats_v3("all_objectives_filtered")
    ...
    if args.online_skip_nonfinite_loss and not math.isfinite(grad_norm):
        optimizer.zero_grad(set_to_none=True)
        return _zero_stats_v3("nonfinite_grad_norm", grad_norm=grad_norm)
    ...
    return OptimizerStepStatsV3(
        ...
        answer_loss=answer_loss_w_sum / answer_w_total if answer_w_total > 0 else 0.0,
        answer_correct_prob=answer_prob_w_sum / answer_w_total if answer_w_total > 0 else 0.0,
        mle_samples_used=mle_samples_used,
        answer_clusters_used=answer_clusters_used,
        answer_wrong_answers_used=answer_wrong_answers_used,
        answer_objects_used=answer_objects_used,
        grad_norm=grad_norm,
        update_applied=True,
        skip_reason="",
...
def _rollout_record_v2(
    *,
    ...
    if objective is None:
        objective_type = "skip"
    elif objective.correct and objective.wrong:
        objective_type = "mixed_answer_contrast"
    elif objective.correct:
        objective_type = "all_correct_mle"
    else:
        objective_type = "all_wrong_answer_contrast"
    record: Dict[str, Any] = {
...
def run_training(args: argparse.Namespace) -> None:
    ...
    if args.hf_data_parallel and device.type == "cuda" and torch.cuda.device_count() > 1:
        ...
        print(
            f"[onesided_v3] enabled DataParallel over {torch.cuda.device_count()} GPUs",
            flush=True,
        )
    ...
    print(
        f"[onesided_v3] dataset_layout={layout} rollout_backend={args.online_rollout_backend} "
        ...
        f"lambda_mle={args.lambda_mle} lambda_answer={args.lambda_answer} "
        ...
        f"answer_tau={args.answer_tau} answer_cluster_weight={args.answer_cluster_weight} "
    ...
    write_metric(
        "run_start",
        {
            ...
            "lambda_answer": float(args.lambda_answer),
            ...
            "answer_tau": float(args.answer_tau),
            "answer_cluster_weight": str(args.answer_cluster_weight),
            "answer_min_cluster_count": int(args.answer_min_cluster_count),
            "answer_max_clusters": int(args.answer_max_clusters),
    ...
    with rollout_log_path.open("w", encoding="utf-8", buffering=1) as rollout_log:
        for sample in source_iter:
            ...
            for idx, sample_obj in enumerate(buffer):
                ...
                objective: Optional[GroupObjective] = GroupObjective(
            ...
            print(
                f"[onesided_v3] rollout_step={rollout_steps}/{total_steps_str} scanned={scanned} "
                f"mixed={mixed_in_rollout} all_correct={all_correct_in_rollout} "
                f"all_wrong={all_wrong_in_rollout} "
            ...
            write_metric(
                "rollout_summary",
                {
                    ...
                    "all_wrong": int(all_wrong_in_rollout),
            ...
            for chunk_start in range(0, len(rollout_objectives), args.online_pairs_per_step):
                ...
                if not stats.update_applied:
                    print(
                        f"[onesided_v3] rollout_step={rollout_steps}/{total_steps_str} "
                ...
                kept_answer_objects += stats.answer_objects_used
                kept_mle_samples += stats.mle_samples_used
                print(
                    f"[onesided_v3] rollout_step={rollout_steps}/{total_steps_str} "
                    f"optimizer_step={updates} mle_loss={stats.mle_loss:.6f} "
                    f"answer_loss={stats.answer_loss:.6f} "
                    f"answer_p={stats.answer_correct_prob:.6f} "
                    f"answer_clusters={stats.answer_clusters_used} "
                    f"wrong_answers={stats.answer_wrong_answers_used} "
                ...
                write_metric(
                    "optimizer_step",
                    {
                        ...
                        "answer_loss": float(stats.answer_loss),
                        "answer_correct_prob": float(stats.answer_correct_prob),
                        "total_loss": float(stats.total_loss),
                        "grad_norm": float(stats.grad_norm),
                        "answer_objects_used": int(stats.answer_objects_used),
                        "mle_samples_used": int(stats.mle_samples_used),
                        "answer_clusters_used": int(stats.answer_clusters_used),
                        "answer_wrong_answers_used": int(stats.answer_wrong_answers_used),
                ...
                if (
                    ...
                    print(f"[onesided_v3] saved checkpoint to {ckpt_dir}", flush=True)
    ...
    print(
        f"[onesided_v3] finished rollout_steps={rollout_steps} optimizer_steps={updates} "
        f"scanned={scanned} kept_answer_objects={kept_answer_objects} "
        f"kept_mle_samples={kept_mle_samples} "
        f"logged_mixed={logged_mixed} logged_all_correct={logged_all_correct} "
        f"logged_all_wrong={logged_all_wrong} "
    ...
    write_metric(
        "run_end",
        {
            ...
            "scanned": int(scanned),
            "kept_answer_objects": int(kept_answer_objects),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "answer-cluster contrast on wrong final answers."
        )
    )
    ...
    parser.add_argument("--lambda_mle", type=float, default=0.3)
    ...
    parser.add_argument("--answer_tau", type=float, default=0.7)
    parser.add_argument(
        "--answer_cluster_weight",
        type=str,
        default="sqrt_count",
        choices=["uniform", "sqrt_count", "count"],
    )
    parser.add_argument("--answer_min_cluster_count", type=int, default=1)
    parser.add_argument("--answer_max_clusters", type=int, default=0)
    parser.add_argument("--answer_use_normalized", type=str2bool, default=True)
    parser.add_argument("--answer_completion_prefix", type=str, default="Answer: ")
    parser.add_argument("--answer_completion_suffix", type=str, default="")
    return parser
...
def main() -> None:
    ...
    validations = [
        ...
        (
            args.lambda_mle < 0 or args.lambda_answer < 0,
            "error: --lambda_mle/--lambda_answer must be >= 0",
        ),
        (
            args.lambda_mle == 0 and args.lambda_answer == 0,
            "error: at least one of --lambda_mle/--lambda_answer must be > 0",
        ),
        ...
        (args.answer_tau <= 0, "error: --answer_tau must be > 0"),
        (args.answer_min_cluster_count < 1, "error: --answer_min_cluster_count must be >= 1"),
        (args.answer_max_clusters < 0, "error: --answer_max_clusters must be >= 0"),
    ...
    if launcher_world_size > 1:
        raise SystemExit(
            "error: train_onesided_v3.py is single-process. Use CUDA_VISIBLE_DEVICES "
