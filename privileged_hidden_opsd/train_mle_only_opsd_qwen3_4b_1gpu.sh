#!/bin/bash
#SBATCH -o logs/mle_only_opsd_4b_1gpu.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
positive_weight_tau=${POSITIVE_WEIGHT_TAU:-1.0}
...
run_root=${RUN_ROOT:-outputs/mle_only_opsd_4b_1gpu/${run_name}}
...
if [[ "${world_size}" -ne 1 ]]; then
  echo "[MLE-ONLY-OPSD][warn] expected 1 GPU for this script, got world_size=${world_size}"
fi
if (( rollout_batch_size % world_size != 0 )); then
  echo "[MLE-ONLY-OPSD][error] rollout_batch_size=${rollout_batch_size} must be divisible by world_size=${world_size}."
  exit 1
fi
if (( logprob_micro_batch_size > 0 )) && (( logprob_micro_batch_size % world_size != 0 )); then
  echo "[MLE-ONLY-OPSD][error] logprob_micro_batch_size=${logprob_micro_batch_size} must be divisible by world_size=${world_size}."
  exit 1
fi

mkdir -p "${run_root}" "${train_out}"

echo "[MLE-ONLY-OPSD] run_root=${run_root}"
echo "[MLE-ONLY-OPSD] model_path=${model_path}"
echo "[MLE-ONLY-OPSD] rollout_batch_size=${rollout_batch_size} rollout_n=${rollout_n}"
echo "[MLE-ONLY-OPSD] lambda_mle=${lambda_mle} positive_weight_mode=${positive_weight_mode}"
echo "[MLE-ONLY-OPSD] mle_include_mixed_correct=true lambda_priv=0 lambda_gt=0"
...
python train_privileged_hidden_opsd.py \
  ...
  --lambda_priv 0.0 \
  --lambda_gt 0.0 \
  --mle_include_mixed_correct true \
  --positive_weight_mode "${positive_weight_mode}" \
  --positive_weight_tau "${positive_weight_tau}" \
  --logprob_micro_batch_size "${logprob_micro_batch_size}" \
  --max_length "${max_length}" \
  --online_sanitize_gradients true \
  --online_grad_element_clip_abs 100.0 \
  --use_lora "${use_lora}" \
  --lora_r "${lora_r}" \
