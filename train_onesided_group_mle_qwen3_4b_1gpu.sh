#!/bin/bash
#SBATCH -o logs/onesided_group_mle_4b_1gpu.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
run_root=${RUN_ROOT:-outputs/onesided_group_mle_4b_1gpu/${run_name}}
...
if [[ "${world_size}" -ne 1 ]]; then
  echo "[ONE-SIDED-GROUP-MLE][warn] expected 1 GPU for this script, got world_size=${world_size}"
fi
if (( rollout_batch_size % world_size != 0 )); then
  echo "[ONE-SIDED-GROUP-MLE][error] rollout_batch_size=${rollout_batch_size} must be divisible by world_size=${world_size}."
  exit 1
fi

mkdir -p "${run_root}" "${train_out}"

echo "[ONE-SIDED-GROUP-MLE] run_root=${run_root}"
echo "[ONE-SIDED-GROUP-MLE] model_path=${model_path}"
echo "[ONE-SIDED-GROUP-MLE] rollout_batch_size=${rollout_batch_size} rollout_n=${rollout_n}"
echo "[ONE-SIDED-GROUP-MLE] lambda_mle=${lambda_mle} lambda_group=${lambda_group} group_tau=${group_tau}"
echo "[ONE-SIDED-GROUP-MLE] weight_type=${one_sided_weight_type} hard_weight_tau=${hard_weight_tau} group_margin=${group_margin} hard_weight_max=${hard_weight_max}"
python train_onesided_group_mle.py \
  --seed "${seed}" \
