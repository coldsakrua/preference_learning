#!/bin/bash
#SBATCH -o logs/pref_1b_1gpu_opsd_thinking.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
opsd_dedup_within_dataset=${OPSD_DEDUP_WITHIN_DATASET:-true}
opsd_enable_cross_dataset_dedup=${OPSD_ENABLE_CROSS_DATASET_DEDUP:-true}
opsd_cross_dedup_root=${OPSD_CROSS_DEDUP_ROOT:-data}
opsd_think_token_format=${OPSD_THINK_TOKEN_FORMAT:-qwen}
...
exp_name=${EXP_NAME:-pref_1b_1gpu_opsd_thinking}
...
python train_preference_opsd_thinking.py \
  ...
  --opsd_processed_parquet "${opsd_processed_parquet}" \
  --opsd_overwrite_processed "${opsd_overwrite_processed}" \
  --opsd_dedup_within_dataset "${opsd_dedup_within_dataset}" \
  --opsd_enable_cross_dataset_dedup "${opsd_enable_cross_dataset_dedup}" \
  --opsd_cross_dedup_root "${opsd_cross_dedup_root}" \
  --opsd_think_token_format "${opsd_think_token_format}"
