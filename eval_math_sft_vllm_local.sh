...
#
# 与 eval_math_vllm_local.sh 参数、默认值、调用方式一致；仅日志名与 OUTPUT_JSON 默认目录不同。
# SFT 评测请设置 CHECKPOINT_DIR 或 LORA_PATH 指向 SFT 的 train/final（或 train），并设 USE_LORA=1。
...
# 1=关闭 Qwen thinking/CoT（--no-thinking）；0=开启 CoT（与 eval_math_vllm_local.py 默认一致）
