#!/bin/bash
#SBATCH -o logs/simpo_4b_1gpu.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
run_root=${RUN_ROOT:-outputs/simpo_4b_1gpu/${run_name}}
...
if [[ "${world_size}" -ne 1 ]]; then
  echo "[SIMPO][warn] expected 1 GPU for this script, got world_size=${world_size}"
fi
if (( rollout_batch_size % world_size != 0 )); then
  echo "[SIMPO][error] rollout_batch_size=${rollout_batch_size} must be divisible by world_size=${world_size} for even split."
  exit 1
fi
if (( logprob_micro_batch_size > 0 )) && (( logprob_micro_batch_size % world_size != 0 )); then
  echo "[SIMPO][error] logprob_micro_batch_size=${logprob_micro_batch_size} must be divisible by world_size=${world_size} for even backprop split."
  exit 1
fi
