...
def build_parser(default_system_prompt: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Enable DeepSpeed engine for HF forward/backward optimization. "
            "Recommended when GPU memory is tight."
        ),
    )
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional DeepSpeed JSON config path. "
            "If empty and --use_deepspeed=true, a memory-optimized ZeRO config is built automatically."
        ),
    )
    parser.add_argument(
        ...
        default=2,
        choices=[1, 2, 3],
        help="ZeRO stage used by auto-generated DeepSpeed config.",
    )
    parser.add_argument(
        "--deepspeed_offload_optimizer",
        type=str2bool,
        default=False,
        help="When true, optimizer states are offloaded to CPU (larger VRAM saving, slower).",
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            "When true and ZeRO-3 is enabled, model params are offloaded to CPU "
            "(largest VRAM saving, can be much slower)."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=50000000,
        help="Reduce bucket size for ZeRO comm (smaller can reduce peak memory at some throughput cost).",
    )
    parser.add_argument(
        "--deepspeed_allgather_bucket_size",
        type=int,
        default=50000000,
        help="Allgather bucket size for ZeRO comm (smaller can reduce peak memory at some throughput cost).",
    )
    parser.add_argument(
        "--deepspeed_stage3_param_persistence_threshold",
        type=int,
        default=100000,
        help="ZeRO-3 param persistence threshold for auto-generated config.",
    )
    parser.add_argument(
        "--deepspeed_stage3_prefetch_bucket_size",
        type=int,
        default=50000000,
        help="ZeRO-3 prefetch bucket size for auto-generated config.",
    )
    parser.add_argument(
        "--hf_data_parallel",
        type=str2bool,
