#!/bin/bash
#SBATCH -o logs/pref_4b_mle_only_1gpu.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
run_root=${RUN_ROOT:-outputs/pref_4b_mle_only_1gpu/${run_name}}
train_out="${run_root}/train"

if [[ "${world_size}" -ne 1 ]]; then
  echo "[PREF-MLE-ONLY][warn] expected 1 GPU for this script, got world_size=${world_size}"
fi
if (( rollout_batch_size % world_size != 0 )); then
  echo "[PREF-MLE-ONLY][error] rollout_batch_size=${rollout_batch_size} must be divisible by world_size=${world_size} for even split."
  exit 1
fi
if (( logprob_micro_batch_size > 0 )) && (( logprob_micro_batch_size % world_size != 0 )); then
  echo "[PREF-MLE-ONLY][error] logprob_micro_batch_size=${logprob_micro_batch_size} must be divisible by world_size=${world_size} for even backprop split."
  exit 1
fi

mkdir -p "${run_root}" "${train_out}"

echo "[PREF-MLE-ONLY] run_root=${run_root}"
echo "[PREF-MLE-ONLY] world_size=${world_size} rollout_batch_per_gpu=$((rollout_batch_size / world_size)) rollout_n=${rollout_n}"
echo "[PREF-MLE-ONLY] use_lora=${use_lora} lora_r=${lora_r} lora_alpha=${lora_alpha}"
echo "[PREF-MLE-ONLY] online mode: only correct trajectories contribute to MLE loss"
