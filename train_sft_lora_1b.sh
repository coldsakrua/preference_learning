#!/bin/bash
#SBATCH -o logs/sft_lora_qwen3_1b.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
# Micro-batch（省显存）；等效批量 �?PER_DEVICE_TRAIN_BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS（单卡）
...
run_root=${RUN_ROOT:-outputs/sft_lora_qwen3_1b/${run_name}}
...
echo "[SFT-LoRA] per_device_train_batch_size=${per_device_train_batch_size} gradient_accumulation_steps=${gradient_accumulation_steps} effective_batch_per_gpu�?{effective_batch} eval_ratio=${eval_ratio}"
